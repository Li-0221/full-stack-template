...
- 这是用于快速验证想法的全栈模板，优先清晰和可维护，不预置 DDD、CQRS、微服务或无消费者的抽象。
- 保留 Dashboard 示例、Users 真实 CRUD、Profile、Security、明暗主题与侧栏折叠、登录和公开错误页。
...
- Users 菜单与路由共用 `frontend/src/lib/router-access.ts` 的管理员规则；前端只控制可见性和导航体验，后端 Dependency 校验管理员入口权限，Service 保护资源关系和业务不变量。
...
- 本地前后端作为本机进程运行，直接连接 dev 服务器 PostgreSQL，不使用数据库 TLS 或 SSH 隧道。dev/prod 部署分别通过 Make 和 deploy 下独立 Compose 管理。
- 仓库内的代码、配置、文档和脚本只使用仓库相对路径，不提交开发机器的绝对路径。
- Markdown 说明使用中文，代码标识、命令和协议字段保持原样。不要提交凭据、`.env`、构建产物、覆盖率或浏览器产物。
- 日常使用 `make check`，完整验证使用 `make check-full`。按改动风险运行最小相关检查；认证、请求层、共享契约和发布边界变更需要扩大验证范围。
- 部署配置与操作以 `deploy/README.md` 为准；新增业务功能参考 `docs/add-feature.md`。
