...
describe('useTableUrlState', () => {
  ...
  it('restores URL-backed filters when browser navigation changes search', async () => {
    const navigate = vi.fn() as Mock<NavigateFn>
    const columnFilters = [
      { columnId: 'title', searchKey: 'q', type: 'string' as const },
    ]
    const { result, rerender } = await renderHook(
      (props?: { search: Record<string, unknown> }) =>
        useTableUrlState({
          search: props?.search ?? {},
          navigate,
          globalFilter: { enabled: true, key: 'filter' },
      ...
      { initialProps: { search: { q: 'first', filter: 'open' } } }
    )

    expect(result.current.columnFilters).toEqual([
      { id: 'title', value: 'first' },
    ])
    expect(result.current.globalFilter).toBe('open')

    await rerender({ search: { q: 'second', filter: 'closed' } })

    expect(result.current.columnFilters).toEqual([
      { id: 'title', value: 'second' },
    ])
    expect(result.current.globalFilter).toBe('closed')
  })
