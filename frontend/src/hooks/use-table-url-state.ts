...
export function useTableUrlState(
  ...
  const columnFilterSearchSignature = JSON.stringify(
    columnFiltersCfg.map((cfg) => [cfg.searchKey, search[cfg.searchKey]])
  )
  const lastColumnFilterSearchSignature = useRef(columnFilterSearchSignature)

  useEffect(() => {
    if (
      lastColumnFilterSearchSignature.current === columnFilterSearchSignature
    ) {
      return
    }
  ...
  }, [columnFilterSearchSignature, initialColumnFilters])
  ...
  const lastUrlGlobalFilter = useRef(urlGlobalFilter)

  useEffect(() => {
    if (lastUrlGlobalFilter.current === urlGlobalFilter) return

    lastUrlGlobalFilter.current = urlGlobalFilter
