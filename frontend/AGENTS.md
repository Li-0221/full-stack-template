...
- 当前公开认证页面只有 `/sign-in`。模板不提供公开注册，账号由管理员通过 Users 创建；密码重置只有在后端邮件流程与安全边界完整接通后才能开放。
...
- 同仓库后端接口默认直接使用 OpenAPI 生成的 request/response 类型，不为生成类型再手写一套字段级 Zod Schema。仅在外部数据、持久化数据、未类型化响应或已有漂移证据时，在第一个消费边界做一次最小运行时校验。
...
- 任务完成默认运行 `pnpm lint` 和覆盖改动行为的最小相关测试，不为每个任务机械运行全量测试。
- 只有修改共享组件、认证、请求层、路由、环境变量、构建/部署配置，或改动跨多个功能、风险较高、准备发布时，才按影响运行完整 `pnpm test` 和 `pnpm build`。
